...
namespace QuantConnect.Tests.Common.Securities
{
    ...
    public class CashTests
    {
        ...
        public void EnsureCurrencyDataFeedAddsSubscription()
        {
            const int quantity = 100;
            const decimal conversionRate = 1 / 100m;
            var cash = new Cash("JPY", quantity, conversionRate, CashBookTests.AccountCurrency);
        ...
        public void EnsureInternalCurrencyDataFeedsForNonUsdQuoteCurrencyGetAdded()
        {
            const int quantity = 100;
            const decimal conversionRate = 1 / 100m;
            var cashJPY = new Cash("JPY", quantity, conversionRate, CashBookTests.AccountCurrency);
            var cashGBP = new Cash("GBP", quantity, conversionRate, CashBookTests.AccountCurrency);
            var cashBook = new CashBook();
        ...
        public void EnsureCurrencyDataFeedForCryptoCurrency()
        {
            var book = new CashBook
            {
                ...
                {"BTC", new Cash("BTC", 100, 6000, CashBookTests.AccountCurrency) },
                {"LTC", new Cash("LTC", 100, 55, CashBookTests.AccountCurrency) },
                {"ETH", new Cash("ETH", 100, 290, CashBookTests.AccountCurrency) },
                {"EUR", new Cash("EUR", 100, 1.2m, CashBookTests.AccountCurrency) },
                {"JPY", new Cash("JPY", 100, 0.0088m, CashBookTests.AccountCurrency) },
                {"XAG", new Cash("XAG", 100, 1275, CashBookTests.AccountCurrency) },
                {"XAU", new Cash("XAU", 100, 17, CashBookTests.AccountCurrency) }
            };
        ...
        public void UpdateModifiesConversionRate()
        {
            const int quantity = 100;
            const decimal conversionRate = 1 / 100m;
            var cash = new Cash("GBP", quantity, conversionRate, CashBookTests.AccountCurrency);
            var cashBook = new CashBook();
